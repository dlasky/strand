...
(function(scope) {
	...
	scope.ItemRecycler = Polymer({
		...
		attached: function() {
			this._tempSlot = this.shadowRoot.querySelector('#temp');
			this._boundTempSlotHandler = this._tempSlotChange.bind(this);
			this._tempSlot.addEventListener('slotchange', this._boundTempSlotHandler);

			Polymer.RenderStatus.afterNextRender(this, function() {
		    	this._tempSlotChange();
			});
		},

		detached: function() {
			if (this._tempSlot && this._boundTempSlotHandler) {
				this._tempSlot.removeEventListener('slotchange', this._boundTempSlotHandler);
			}
		},

		_tempSlotChange: function(e) {
			if (e) {
				console.log('here: ', e);
			} else {
				console.log(
					'lightDOM was added, but no \'slotchange\' event: ',
					this._tempSlot.assignedNodes()
				);
			}
		},
